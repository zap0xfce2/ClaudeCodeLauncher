...
Curses-basierter Terminal-Launcher für das Management von Claude Code Sessions. Vereinfacht das Backup, Wiederherstellen und Wechseln zwischen verschiedenen ClaudeHome-Verzeichnissen.

## Konzept: ClaudeHome

**ClaudeHome** ist das Arbeitsverzeichnis einer Claude-Session – kein spezielles Konfigurationsverzeichnis. Es enthält projektspezifische Dateien wie `CLAUDE.md`, `Plan.md` oder `settings.local.json` und kann an beliebiger Stelle liegen (z. B. `/Users/alice/projects/mein-projekt` oder `/Volumes/RamDisk/work`).

> **Nicht verwechseln mit `~/.claude`** – das ist das globale Konfigurationsverzeichnis von Claude Code. Der Launcher verwaltet dagegen projektspezifische Arbeitsverzeichnisse, von denen es beliebig viele geben kann.

## Features

- **Session-Management** – Neue Session starten oder bestehende fortsetzen
- **Export / Import** – ClaudeHome als Backup sichern und wiederherstellen
...
- **Shell-Zugang** – Terminal im ClaudeHome-Verzeichnis öffnen
...
`config.yaml` wird standardmäßig im selben Verzeichnis wie `ClaudeCodeLauncher.py` angelegt. Das geöffnete ClaudeHome-Verzeichnis wird im Hauptmenü oben rechts angezeigt.
...
- `--export /tmp/datei.json` → Dateiauswahl aus ClaudeHome, Export als einzelne Datei
- `--export /tmp/backup` → Folder-Modus (komplettes ClaudeHome)

### Alle CLI-Argumente

| Argument | Beschreibung |
|---|---|
| `claude_home` | Pfad zum Arbeitsverzeichnis der Claude-Session (ClaudeHome) **(Pflicht)** |
| `--export PATH` | Exportiert ClaudeHome direkt zum angegebenen Pfad |
| `--import PATH` | Importiert ClaudeHome direkt vom angegebenen Pfad |
...
| `ask_for_reset` | bool | `true` | Nach Folder-Export: Fragen ob ClaudeHome zurückgesetzt werden soll |
